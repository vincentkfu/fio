...
.. option:: readwrite=str, rw=str
	...
	the :option:`rw_sequencer` option. Note ``verify`` will not work in this
	scenario.
...
.. option:: norandommap
	...
	used with :option:`verify` then random seed within the special header at the
	beginning of each block will not be verified. If this option is used with
	:option:`verify` and multiple blocksizes (via :option:`bsrange`), only intact
	blocks are verified, i.e., partially-overwritten blocks are ignored.
