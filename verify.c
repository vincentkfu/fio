...
static int verify_io_u_pattern(struct verify_header *hdr, struct vcont *vc)
{
	...
	dprint(FD_VERIFY, "get_hdr_inc = %u, header_size = %u, pattern_size = %u\n", get_hdr_inc(td, io_u), header_size, pattern_size);
	dprint(FD_VERIFY, "len = %u, mod = %u, pattern_interval_mod = %u\n", len, mod, pattern_interval_mod);

	while (bytes_done < len) {
		dprint(FD_VERIFY, "io_u->offset = %llu, bytes_done = %u, bytes_todo = %u, mod = %u\n", io_u->offset, bytes_done, bytes_todo, mod);
		if (td->o.verify_pattern_interval) {
			(void)paste_format_inplace(pattern, pattern_size,
					td->o.verify_fmt, td->o.verify_fmt_sz,
					io_u);
